...
from ape_ledger._cli import _get_container
...
def _clean_up(runner):
    runner.invoke(cli, ["ledger", "remove", TEST_ALIAS], input="y")
...
def test_list(runner, existing_account):
    ...
    assert TEST_ADDRESS.lower() in result.output
...
def test_add(runner, mock_device_connection):
    ...
    assert result.exit_code == 0, result.output
    assert f"successfully added with alias '{TEST_ALIAS}'" in result.output
...
def test_add_when_hd_path_specified(runner, mock_ethereum_app, mock_device_connection, mock_account):
    ...
    expected_path = _get_container().data_folder.joinpath(f"{TEST_ALIAS}.json")
...
def test_remove(runner, existing_account):
    result = runner.invoke(cli, ["ledger", "remove", TEST_ALIAS], input="y")
    assert result.exit_code == 0, result.output
    assert f"SUCCESS: Account '{TEST_ALIAS}' has been removed" in result.output


def test_remove_account_not_exists(runner):
    result = runner.invoke(cli, ["ledger", "remove", TEST_ALIAS], input="y")
    assert result.exit_code == 1
